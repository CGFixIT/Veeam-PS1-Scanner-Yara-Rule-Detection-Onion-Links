...
function Write-Log {
    ...
    Write-Host $logEntry
    # FIX: Specify -Encoding UTF8 so non-ASCII characters in matched strings,
    # log separators, and level labels are written correctly on PS5.1.
    # PS5.1 Add-Content defaults to the system ANSI code page without this.
...
function Export-ScanResults {
    ...
    # Export to JSON
    # FIX: Specify -Encoding UTF8 so that non-ASCII characters in matched YARA
    # strings (e.g. binary data, onion link characters) are written correctly.
    # PS5.1 Set-Content defaults to the system ANSI code page without -Encoding.
    $jsonOutput = @{
